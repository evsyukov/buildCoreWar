...
//	token = pass_new_line_token(token);
	...
	{
		if (token->type_token == NEW_LINE)
			token = token->next;
//		else if (token->type_token == COMMENT) {
//			token = token->next;
//			if (token->type_token != STRING || token->next->type_token != NEW_LINE) {
//				print_error_and_exit();
//			}
//			token = token->next;
//		}
		else if (token->type_token == LABEL) {
//			node_str = ft_strsub(token->str, 0, ft_strlen(token->str) - 1);
			MFAIL((node_str = ft_strsub(token->str, 0,
