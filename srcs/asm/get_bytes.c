								...
								char *str)
{
	...
	hash = hash_query(asm_node->h_table, str);
	if (hash == NULL)
						...
						t_token *token)
{
	...
	if (type_token == DIRECT_LABEL)
		index = 2;
//	node_str = ft_strsub(token->str, index, len - index);
	MFAIL((node_str = ft_strsub(token->str, index, len - index)));
	if (node_str == NULL)
		print_error_and_exit();
...
static size_t	get_register_code(char *str)
{
	...
	len = ft_strlen(str);
//	str_sub = ft_strsub(str, 1, len - 1);
	MFAIL((str_sub = ft_strsub(str, 1, len - 1)));
	if (str_sub == NULL)
		print_error_and_exit();
