...
extern unsigned long	g_ml;
extern unsigned long	g_fr;

# define FCNT(x) x; g_fr++
# define MFAIL(x) if (!x) err_exit(); else g_ml++
...
typedef struct s_asm
{
	...
	t_instr_list	instr_list; // список инструкций (выделяется память)
	...
	char			*hex_code; // массив байт кода чемпиона (выделяется память)
}				t_asm;
...
int				gnl_with_flag_endline(const int fd, char **line, int *is_endline);
...
void			parse_token_list(t_asm *asm_node);
